...
QKxRemoteAbilityModel::QKxRemoteAbilityModel(QObject *parent)
...
QKxRemoteAbilityModel::~QKxRemoteAbilityModel()
{

}

void QKxRemoteAbilityModel::setServiceFlag(qint64 devid, quint64 flag)
{
    if(m_serviceFlag == flag && devid == m_deviceId) {
        return;
    }
    m_deviceId = devid;
    beginResetModel();
    m_serviceFlag = flag;
    endResetModel();
    qDebug() << "setServiceFlag:" << flag;
}

void QKxRemoteAbilityModel::onServiceFlagArrived(qint64 devid, quint64 flag)
{
    if(devid != m_deviceId) {
        return;
    }
...
int QKxRemoteAbilityModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid()){
        return 0;
    }
...
QModelIndex QKxRemoteAbilityModel::sibling(int row, int column, const QModelIndex &idx) const
{
...
QVariant QKxRemoteAbilityModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    return QAbstractListModel::headerData(section, orientation, role);
}

QVariant QKxRemoteAbilityModel::data(const QModelIndex &index, int role) const
{
    if (index.row() < 0) {
        return QVariant();
    }
    ...
    }else if(role == ROLE_DEVICE_ID) {
        return m_deviceId;
    }
    return QVariant();
}

bool QKxRemoteAbilityModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    return false;
}

Qt::ItemFlags QKxRemoteAbilityModel::flags(const QModelIndex &index) const
{
    return Qt::ItemIsSelectable | Qt::ItemIsEnabled;;
}

bool QKxRemoteAbilityModel::insertRows(int row, int count, const QModelIndex &parent)
{
    qDebug() << "insertRow";
    return false;
}

bool QKxRemoteAbilityModel::removeRows(int row, int count, const QModelIndex &parent)
{
...
void QKxRemoteAbilityModel::sort(int column, Qt::SortOrder order)
{
    return QAbstractListModel::sort(column, order);
}

int QKxRemoteAbilityModel::columnCount(const QModelIndex &parent) const
{
    return 1;
}

Qt::DropActions QKxRemoteAbilityModel::supportedDropActions() const
{
